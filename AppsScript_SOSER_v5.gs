/*******************************************************************
 * SOSER · Backend Google Apps Script (v5)
 * -----------------------------------------------------------------
 * v5: los encabezados se auto-reparan. Si la app agrega columnas nuevas,
 *     la hoja se amplía sola sin mover los datos ya escritos.
 *
 * AGREGA (sin tocar nada de "Agregar Caso"):
...
function pintarEncabezados(sh, headers, extraHeaders) {
  var nBase = headers.length, nExtra = extraHeaders.length;

  // Fila 1: bloques temáticos, igual que la plantilla Excel
  ...
  try { rngExtraBloque.breakApart(); } catch (e) {}
  rngExtraBloque.merge();
  rngExtraBloque.setValue('DATOS ADICIONALES DE LA APP · NO existen en la plantilla original · no copiar a la matriz')
  ...
  sh.getRange(2, 1, 1, nBase).setValues([headers])
    ...
    .setWrap(true).setVerticalAlignment('middle').setHorizontalAlignment('center').setFontSize(9);
  ...
  sh.setRowHeight(2, 86);
  for (var c = 1; c <= nBase + nExtra; c++) sh.setColumnWidth(c, c <= 4 ? 175 : (c <= nBase ? 130 : 200));

  // Línea divisoria entre la matriz original y lo agregado por la app
  sh.getRange(1, nBase, sh.getMaxRows(), 1)
    .setBorder(null, null, null, true, null, null, '#1769AA', SpreadsheetApp.BorderStyle.SOLID_THICK);
}

function hojaFicha(ss, headers, extraHeaders) {
  var sh = ss.getSheetByName(HOJA_FICHA);
  var nBase = headers.length, nExtra = extraHeaders.length;
  var total = nBase + nExtra;

  if (!sh) {
    sh = ss.insertSheet(HOJA_FICHA);
    if (sh.getMaxColumns() < total) sh.insertColumnsAfter(sh.getMaxColumns(), total - sh.getMaxColumns());
    pintarEncabezados(sh, headers, extraHeaders);
    return sh;
  }

  // La hoja ya existía: si se agregaron columnas nuevas en la app, se amplía
  // sin mover ni tocar los datos que ya estaban escritos.
  if (sh.getMaxColumns() < total) sh.insertColumnsAfter(sh.getMaxColumns(), total - sh.getMaxColumns());
  var actual = sh.getRange(2, 1, 1, total).getValues()[0];
  var faltan = false;
  for (var i = 0; i < total; i++) {
    var esperado = i < nBase ? headers[i] : extraHeaders[i - nBase];
    if (String(actual[i]).trim() !== String(esperado).trim()) { faltan = true; break; }
  }
  if (faltan) pintarEncabezados(sh, headers, extraHeaders);
  return sh;
}
